def convert number
  if number.to_i <= 3000
    ...
    'IL'
    ('X' * ((number.to_i%50).to_i / 10).to_i).to_s + #need to exclude 49 from this 
    'IX' * ((number.to_i%10).to_i / 9).to_i).to_s +
    ('V' * ((number.to_i%10).to_i / 5).to_i).to_s + #need to exclude 9 from this
    ('IV' * ((number.to_i%5).to_i / 4).to_i).to_s +
    ('I' * (((number.to_i%5).to_i) - ((4 * ((number.to_i%5).to_i / 4))).to_i)).to_s #how do I make this only for three or less?
